...
pub struct BrowserExecutionState {
    ...
    promise_resolve_guards: HashSet<u64>,
    next_promise_resolve_guard_id: u64,
    ...
    pending_generator_resume_expression: Option<ExpressionContinuation>,
    pending_generator_resume_if: Option<IfStatement>,
    ...
    pending_async_resume_expression: Option<ExpressionContinuation>,
...
impl JsObject {
    ...
    pub fn set_setter(&mut self, key: impl Into<String>, setter: JsValue) {
        let k = key.into();
        // Merge into existing accessor if one already exists (e.g. get+set pair).
        let entry = self.properties.entry(k).or_insert(Property::Accessor {
            get: None,
            set: None,
            enumerable: false,
            configurable: true,
        });
        if let Property::Accessor { set, .. } = entry {
            *set = Some(setter);
        }
    }
...
pub(crate) struct GeneratorState {
    ...
    stack: Vec<StackFrame>,
    /// Suspended expression continuation, currently direct yield in call arguments.
    pending_expression: Option<ExpressionContinuation>,
...
impl BrowserExecutionState {
    ...
    fn obj_has_property(rc: &Rc<RefCell<JsObject>>, key: &str) -> bool {
        if rc.borrow().has_own(key) {
            return true;
        }
        let proto = rc.borrow().prototype.clone();
        proto
            .as_ref()
            .is_some_and(|proto| Self::obj_has_property(proto, key))
    }
    ...
    fn drain_and_run_microtasks(&mut self) {
        while !self.pending_microtasks.is_empty() {
            ...
            match task {
                ...
                PendingMicrotask::PromiseResolveThenable {
                    promise,
                    thenable,
                    then,
                } => {
                    self.run_promise_resolve_thenable_job(promise, thenable, then);
                }
    ...
    fn next_promise_guard_id(&mut self) -> u64 {
        self.next_promise_resolve_guard_id += 1;
        self.next_promise_resolve_guard_id
    }

    fn promise_capability_functions(
        &mut self,
        promise: &Rc<RefCell<PromiseState>>,
    ) -> (JsValue, JsValue, u64) {
        let promise_value = JsValue::Promise(promise.clone());
        let guard_id = self.next_promise_guard_id();
        let guard = JsValue::Number(guard_id as f64);
        let resolve = JsValue::BoundHostFunction {
            name: "PromiseCapability.resolve".to_owned(),
            this_arg: Box::new(JsValue::Undefined),
            bound_args: vec![promise_value.clone(), guard.clone()],
        };
        let reject = JsValue::BoundHostFunction {
            name: "PromiseCapability.reject".to_owned(),
            this_arg: Box::new(JsValue::Undefined),
            bound_args: vec![promise_value, guard],
        };
    ...
    fn promise_resolve_input(&mut self, value: JsValue) -> JsValue {
        if matches!(value, JsValue::Promise(_)) {
            return value;
        }
        let promise = Self::pending_promise();
        self.promise_resolve_to(&promise, value);
        JsValue::Promise(promise)
    }

    fn promise_resolve_to(&mut self, promise: &Rc<RefCell<PromiseState>>, value: JsValue) {
        if let JsValue::Promise(inner) = &value {
            if Rc::ptr_eq(promise, inner) {
                self.settle_promise(
                    promise,
                    PromiseStatus::Rejected(Self::make_error_obj(
                        "TypeError",
                        "Promise resolved with itself".to_owned(),
                    )),
                );
                return;
            }
        ...
        match value.clone() {
            ...
            JsValue::Object(ref rc) => {
                ...
                if Self::is_callable_value(&then) {
                    self.pending_microtasks
                        .push(PendingMicrotask::PromiseResolveThenable {
            ...
            JsValue::Function(ref func) => {
                let then = self.function_get(func, value.clone(), "then");
                if let Some(EarlyExit::Throw(error)) = self.early_exit.take() {
                    self.settle_promise(promise, PromiseStatus::Rejected(error));
                    return;
                }
            ...
            _ => self.settle_promise(promise, PromiseStatus::Fulfilled(value)),
        }
    }
    ...
    fn run_promise_resolve_thenable_job(
        ...
        self.call_value(then, thenable, vec![resolve, reject]);
        if let Some(EarlyExit::Throw(error)) = self.early_exit.take() {
            ...
            PromiseReaction::Then {
                ...
                if let Some(func) = handler {
                    ...
                    if let Some(EarlyExit::Throw(error)) = self.early_exit.take() {
                        ...
                        self.promise_resolve_to(&chained, result);
                    }
    ...
    fn execute_statement(&mut self, statement: &Statement) {
        self.execute_statement_with_label(statement, None);
    }

    fn exit_matches_label(target: &Option<String>, active_label: Option<&str>) -> bool {
        match (target.as_deref(), active_label) {
            (None, None) => true,
            (None, Some(_)) => true,
            (Some(target), Some(label)) => target == label,
            (Some(_), None) => false,
        }
    }

    fn exit_matches_exact_label(target: &Option<String>, label: &str) -> bool {
        matches!(target.as_deref(), Some(target) if target == label)
    }

    fn execute_statement_with_label(&mut self, statement: &Statement, active_label: Option<&str>) {
        ...
        match statement {
            ...
            Statement::While(statement) => {
                ...
                loop {
                    ...
                    match self.early_exit {
                        Some(EarlyExit::Break(ref label))
            ...
            Statement::ClassDeclaration(decl) => {
                let decl = decl.clone();
                let class_val = self.execute_class_decl(&decl);
                let name = decl.name.clone();
                self.set_local(&name, class_val);
            }
            ...
            Statement::Break(stmt) => {
                self.early_exit = Some(EarlyExit::Break(stmt.label.clone()));
            }
            Statement::Continue(stmt) => {
                self.early_exit = Some(EarlyExit::Continue(stmt.label.clone()));
            }
            Statement::Labeled(stmt) => {
                self.execute_statement_with_label(&stmt.body, Some(&stmt.label));
                if matches!(
                    &self.early_exit,
                    Some(EarlyExit::Break(label))
                        if Self::exit_matches_exact_label(label, &stmt.label)
                ) {
                    self.early_exit = None;
                }
            }
            ...
            Statement::Switch(stmt) => {
                self.execute_switch(stmt, active_label);
            }
            Statement::Empty => {}
        }
    }
    ...
    fn execute_switch(&mut self, stmt: &SwitchStatement, active_label: Option<&str>) {
        let stmt = stmt.clone();
        ...
        if matches!(
            &self.early_exit,
            Some(EarlyExit::Break(label)) if Self::exit_matches_label(label, active_label)
        ) {
            self.early_exit = None;
        }
    }
        ...
        for stmt in stmts {
            match stmt {
                ...
                Statement::Labeled(labeled) => {
                    self.hoist_function_declarations(std::slice::from_ref(labeled.body.as_ref()));
                }
                _ => {}
            }
        }
                    ...
                    if matches!(
                        ...
                        if args.len() == 1 {
                            if let JsValue::Number(n) = args[0] {
                                // Spec: if n is not a valid uint32, throw RangeError.
                                let u = n as u32;
                                if (u as f64) != n {
                                    self.early_exit = Some(EarlyExit::Throw(Self::make_error_obj(
                                        "RangeError",
                                    ...
                                    // (sparse arrays not yet supported).
                                    JsValue::Array(vec![])
                                } else {
                                    JsValue::Array(vec![JsValue::Undefined; u as usize])
                                }
                            } else {
                                JsValue::Array(args)
                            }
    ...
    fn execute_call(&mut self, callee: &Expression, arguments: &[Expression]) -> JsValue {
        ...
        if matches!(callee, Expression::Super) {
            ...
            // The super ctor is injected as __super_ctor__ in the function frame.
            if let Some(super_val) = self.get_binding("__super_ctor__") {
                match super_val {
                    JsValue::Function(ctor) => {
                        self.call_function_with_this(ctor, args, this_val);
                    }
                    JsValue::HostFunction(_) => {
                        // Built-in super — no-op for now.
                    }
                    _ => {}
                }
            }
        ...
        self.call_evaluated_callee(func_val, args)
    }
    ...
    fn call_object_static(&mut self, name: &str, args: Vec<JsValue>) -> JsValue {
        match name {
            ...
            "isExtensible" => JsValue::Boolean(true),
            ...
            "forEach" => {
                ...
                if let Some(JsValue::Function(func)) = cb {
                    ...
                    for (i, item) in arr.into_iter().enumerate() {
                        ...
                        if matches!(
                            self.early_exit,
                            Some(EarlyExit::Break(_)) | Some(EarlyExit::Continue(_))
                        ) {
                            self.early_exit = None;
                        }
                        if self.early_exit.is_some() {
                ...
                if let Some(JsValue::Function(func)) = cb {
                    ...
                    for (i, item) in arr.into_iter().enumerate() {
                        ...
                        let keep = self.call_function(
                            func.clone(),
                            vec![item.clone(), JsValue::Number(i as f64), arr_val.clone()],
                        );
                        if self.early_exit.is_some() {
                            break;
                        }
                ...
                if let Some(JsValue::Function(func)) = cb {
                    ...
                    for (i, item) in arr.into_iter().enumerate().skip(start) {
                        ...
                        acc = self.call_function(
                            func.clone(),
                            vec![acc, item, JsValue::Number(i as f64), arr_val.clone()],
                        );
                        if self.early_exit.is_some() {
                            break;
                        }
    ...
    /// Build a class constructor JsValue from a ClassDeclaration.
    /// Used by both `Statement::ClassDeclaration` and `Expression::Class`.
    fn execute_class_decl(&mut self, decl: &ClassDeclaration) -> JsValue {
        // 1. Resolve superclass.
        let super_ctor_val: Option<JsValue> = decl.superclass.as_ref().map(|name| {
            // Support dotted names like "Foo.Bar" via simple chained lookup.
            let mut parts = name.splitn(2, '.');
            let base = parts.next().unwrap_or("");
            let val = self.get_binding(base).unwrap_or(JsValue::Undefined);
            if let Some(prop) = parts.next() {
                if let JsValue::Object(rc) = &val {
                    self.obj_get(rc, prop)
                } else {
                    val
                }
            } else {
        ...
        if let Some(sp) = super_proto {
            proto_rc.borrow_mut().prototype = Some(sp);
        }
        ...
        // 5. Add instance methods to prototype.
        {
            ...
            for method in &instance_methods {
                ...
                match kind {
                    ...
                    MethodKind::Set => proto.set_setter(method.name.clone(), mval),
                    MethodKind::Method => proto.set(method.name.clone(), mval),
                }
            }
        }
            ...
            MemberProperty::Named(property) => {
                ...
                let result = match receiver.clone() {
                    ...
                    JsValue::BoundHostFunction { .. } => match property.as_str() {
                        ...
                        "name" => JsValue::String(String::new()),
    ...
    fn eval_call_args_with_expression_continuation(
        ...
        callee: JsValue,
        arguments: &[Expression],
        mut prefix: Vec<JsValue>,
    ) -> Option<Vec<JsValue>> {
        for (i, arg) in arguments.iter().enumerate() {
            let before_len = prefix.len();
            if let Expression::Spread(inner) = arg {
                let val = self.execute_expression(inner);
                if let JsValue::Array(items) = val {
                    prefix.extend(items);
                ...
                prefix.push(self.execute_expression(arg));
            }
    ...
    fn resume_expression_continuation(
        ...
        match continuation {
            ...
            ExpressionContinuation::Call {
                ...
                prefix.push(resume_value);
                match self.eval_call_args_with_expression_continuation(
                    callee.clone(),
                    &remaining,
                    prefix,
                ) {
    ...
    fn call_evaluated_callee(&mut self, func_val: JsValue, args: Vec<JsValue>) -> JsValue {
    ...
    fn resume_generator_body(
        ...
        if let Some(if_stmt) = pending_if.take() {
            let mut condition_value = resume;
            if let Some(expression) = pending_expression.take() {
                condition_value = self.resume_expression_continuation(expression, condition_value);
                if let Some(yielded) = self.pending_generator_yield.take() {
                    let stack = std::mem::replace(&mut self.stack, saved_stack);
                    let pending_expression = self.pending_generator_resume_expression.take();
                    let mut state = rc.borrow_mut();
                    state.stack = stack;
                    state.index = index;
                    state.body = body;
                    ...
                    state.pending_if = Some(if_stmt);
                    return GeneratorState::result(yielded, false);
                }
            }
            let selected = if Self::is_truthy(&condition_value) {
        ...
        loop {
            ...
            match statement.clone() {
                ...
                Statement::If(if_stmt) => {
                    ...
                    if self.pending_generator_yield.is_some() {
                        let yielded = self
                            .pending_generator_yield
                            .take()
                            .expect("generator if condition should have yielded");
                        self.pending_generator_resume_if = Some(if_stmt);
                        let stack = std::mem::replace(&mut self.stack, saved_stack);
                        let pending_if = self.pending_generator_resume_if.take();
                        let mut state = rc.borrow_mut();
                        state.stack = stack;
                        state.index = index;
                        state.body = body;
                ...
                Statement::While(while_stmt) => {
                    ...
                    frames.push(AsyncStatementFrame {
                        ...
                        pop_scope: false,
                        kind: AsyncFrameKind::Loop {
                            cursor: LoopCursor::While(while_stmt.clone()),
                        },
                    });
                ...
                Statement::DoWhile(do_while_stmt) => {
                    ...
                    frames.push(AsyncStatementFrame {
                        ...
                        pop_scope: false,
                        kind: AsyncFrameKind::Loop {
                            cursor: LoopCursor::DoWhile(do_while_stmt.clone()),
                        },
                    });
                    body = vec![*do_while_stmt.body];
                    index = 0;
                    continue;
                }
            ...
            if let Some(yielded) = self.pending_generator_yield.take() {
                ...
                let pending_if = self.pending_generator_resume_if.take();
                let next_index = if pending_expression.is_some()
                    || pending_binding.is_some()
                    || pending_assignment.is_some()
                    || pending_if.is_some()
                {
                    index
                } else {
            ...
            match self.early_exit.take() {
                ...
                Some(EarlyExit::Continue(label)) if label.is_none() => {
                    let mut handled = false;
                    while let Some(frame) = frames.pop() {
                        if frame.pop_scope {
                            self.stack.pop();
    ...
    fn begin_for_in_loop_cursor(&mut self, statement: ForInStatement) -> Option<LoopCursor> {
        let object = self.execute_expression(&statement.object);
        let keys: Vec<String> = match object {
            JsValue::Object(rc) => rc.borrow().own_enumerable_keys(),
            _ => vec![],
        };
        let first_key = keys.first()?.clone();
        self.stack.push(StackFrame::block_scope());
        self.bind_iteration_value(
            &statement.binding,
            statement.binding_kind,
            JsValue::String(first_key),
        );
        Some(LoopCursor::ForIn {
            statement,
            keys,
            next_index: 1,
        })
    ...
    fn call_function_with_this(
        ...
        for field in instance_fields {
            ...
            if let Some(JsValue::Object(rc)) = self.get_binding("this") {
                ...
                rc.borrow_mut().set(field_name, val);
            }
        }
        let arguments_obj = Self::build_arguments_object(&args);
        self.bind_params(&func.params, args);
    ...
    fn next_loop_cursor_body(&mut self, cursor: &mut LoopCursor) -> LoopCursorStep {
        match cursor {
            LoopCursor::While(statement) => {
                let condition = self.execute_expression(&statement.test);
                if self.expression_suspended() || self.early_exit.is_some() {
                    return LoopCursorStep::Suspended;
                }
                if Self::is_truthy(&condition) {
                    LoopCursorStep::Body(*statement.body.clone())
                } else {
            ...
            LoopCursor::For(statement) => {
                if let Some(update) = &statement.update {
                    self.execute_expression(update);
                    if self.early_exit.is_some()
                        || self.pending_async_await.is_some()
                        || self.pending_generator_yield.is_some()
                    {
                        return LoopCursorStep::Suspended;
                    }
                }
            ...
            LoopCursor::ForIn {
                ...
                self.bind_iteration_value(
                    &statement.binding,
                    statement.binding_kind,
                    JsValue::String(keys[*next_index].clone()),
                );
                *next_index += 1;
                LoopCursorStep::Body(*statement.body.clone())
            }
            LoopCursor::ForOf {
                ...
                let Some(value) = self.iterator_next_value(iterator) else {
                    return LoopCursorStep::Done;
                };
                self.stack.push(StackFrame::block_scope());
    ...
    fn resume_async_continuation(&mut self, id: u64, status: PromiseStatus) {
        ...
        if !rejected_resume {
            if let Some(if_stmt) = continuation.pending_if.take() {
                let mut condition_value = resume_value;
                if let Some(expression) = continuation.pending_expression.take() {
                    condition_value =
                        self.resume_expression_continuation(expression, condition_value);
                    if let Some(awaited) = self.pending_async_await.take() {
                        continuation.stack = std::mem::replace(&mut self.stack, saved_stack);
                        continuation.pending_expression =
                            self.pending_async_resume_expression.take();
                        continuation.pending_if = Some(if_stmt);
                        self.attach_promise_reaction(
                            &awaited,
                            PromiseReaction::AsyncContinuation { id },
                        );
            ...
            } else if let Some(loop_header) = continuation.pending_loop_header.take() {
                ...
                if let Some(expression) = continuation.pending_expression.take() {
                    ...
                    if let Some(awaited) = self.pending_async_await.take() {
                        ...
                        continuation.pending_expression =
                            self.pending_async_resume_expression.take();
                        continuation.pending_loop_header = Some(loop_header);
                        self.attach_promise_reaction(
                            &awaited,
                            PromiseReaction::AsyncContinuation { id },
                        );
                ...
                match loop_header {
                    AsyncPendingLoopHeader::ForInit(for_stmt)
                    | AsyncPendingLoopHeader::ForTest(for_stmt) => {
                        let mut enter_body = true;
                        if let Some(test) = &for_stmt.test {
                            let condition = self.execute_expression(test);
                            if let Some(awaited) = self.pending_async_await.take() {
                                ...
                                continuation.pending_loop_header =
                                    Some(AsyncPendingLoopHeader::ForTest(for_stmt));
                                self.attach_promise_reaction(
                                    &awaited,
                                    PromiseReaction::AsyncContinuation { id },
                                );
                        ...
                        if enter_body {
                            ...
                            continuation.body = vec![*for_stmt.body];
                            continuation.index = 0;
                        }
                    }
                    AsyncPendingLoopHeader::ForUpdate(mut cursor) => {
                        let step = match &mut cursor {
                            LoopCursor::For(statement) => {
                                if let Some(test) = &statement.test {
                                    let condition = self.execute_expression(test);
                    ...
                    AsyncPendingLoopHeader::ForInObject(for_in_stmt) => {
                        let keys: Vec<String> = match value {
                            JsValue::Object(rc) => rc.borrow().own_enumerable_keys(),
                            _ => vec![],
                        };
                        if let Some(first_key) = keys.first().cloned() {
                            ...
                            continuation.body = vec![*for_in_stmt.body];
                            continuation.index = 0;
                        } else {
                            continuation.index += 1;
                        }
                    }
                    AsyncPendingLoopHeader::ForOfIterable(for_of_stmt) => {
                        if let Some(mut iterator) = self.get_iterator_record(value) {
                            if let Some(first_value) = self.iterator_next_value(&mut iterator) {
                                self.stack.push(StackFrame::block_scope());
                                self.bind_iteration_value(
            ...
            continuation.pending_expression = None;
            continuation.pending_binding = None;
            continuation.pending_assignment = None;
            continuation.pending_if = None;
            continuation.pending_loop_header = None;
        ...
        loop {
            ...
            match statement.clone() {
                ...
                Statement::If(if_stmt) => {
                    let condition = self.execute_expression(&if_stmt.test);
                    if self.pending_async_await.is_some() {
                        let Some(awaited) = self.pending_async_await.take() else {
                            unreachable!();
                        };
                        continuation.stack = std::mem::replace(&mut self.stack, saved_stack);
                        continuation.pending_expression =
                ...
                Statement::DoWhile(do_while_stmt) => {
                    ...
                    continuation.body = vec![*do_while_stmt.body];
                    continuation.index = 0;
                    continue;
                }
                Statement::For(for_stmt) => {
                    ...
                    if let Some(awaited) = self.pending_async_await.take() {
                        ...
                        continuation.pending_expression =
                            self.pending_async_resume_expression.take();
                        continuation.pending_binding = self.pending_async_resume_binding.take();
                        continuation.pending_assignment =
                            self.pending_async_resume_assignment.take();
                        continuation.pending_loop_header =
                            Some(AsyncPendingLoopHeader::ForInit(for_stmt));
                        self.attach_promise_reaction(
                            &awaited,
                            PromiseReaction::AsyncContinuation { id },
                        );
                    ...
                    let Some(cursor) = self.begin_for_in_loop_cursor(for_in_stmt.clone()) else {
                        if let Some(awaited) = self.pending_async_await.take() {
                            ...
                            continuation.pending_expression =
                                self.pending_async_resume_expression.take();
                            continuation.pending_loop_header =
                                Some(AsyncPendingLoopHeader::ForInObject(for_in_stmt));
                            self.attach_promise_reaction(
                                &awaited,
                                PromiseReaction::AsyncContinuation { id },
                            );
                    ...
                    let Some(cursor) = self.begin_for_of_loop_cursor(for_of_stmt.clone()) else {
                        if let Some(awaited) = self.pending_async_await.take() {
                            ...
                            continuation.pending_expression =
                                self.pending_async_resume_expression.take();
                            continuation.pending_loop_header =
                                Some(AsyncPendingLoopHeader::ForOfIterable(for_of_stmt));
                            self.attach_promise_reaction(
                                &awaited,
                                PromiseReaction::AsyncContinuation { id },
                            );
            ...
            if let Some(awaited) = self.pending_async_await.take() {
                let pending_expression = self.pending_async_resume_expression.take();
                ...
                continuation.pending_expression = pending_expression;
        ...
        match name {
                ...
                if let Some(JsValue::Promise(promise)) = args.first() {
                    let guard = args
                        ...
                        .and_then(|value| match value {
                            JsValue::Number(id) => Some(*id as u64),
                            _ => None,
                        })
                        .unwrap_or(0);
                    if guard != 0 && !self.promise_resolve_guards.insert(guard) {
                        return JsValue::Undefined;
                    }
            ...
            "PromiseCapability.reject" => {
                if let Some(JsValue::Promise(promise)) = args.first() {
                    ...
                    let reason = args.get(2).cloned().unwrap_or(JsValue::Undefined);
            ...
            "Reflect.apply" => {
                ...
                if !Self::is_callable_value(&target) {
                    self.early_exit = Some(EarlyExit::Throw(Self::make_error_obj(
                        "TypeError",
                        "Reflect.apply target is not callable".to_owned(),
                    )));
                    return JsValue::Undefined;
                }
                ...
                let arguments = args
                    .get(2)
                    .map(|value| self.collect_iterable_values(value.clone()))
                    .unwrap_or_default();
                self.call_value(target, this_arg, arguments)
            }
            "Reflect.construct" => {
            ...
            "Object.prototype.hasOwnProperty" => {
                ...
                match &this_arg {
                    ...
                    JsValue::BoundHostFunction { .. } => {
                        JsValue::Boolean(matches!(key.as_str(), "name" | "length"))
                    }
            ...
            // Always reads length first (to trigger any getter side effects), then
            // checks callback is callable (TypeError if not), then iterates.
    ...
    fn array_like_parts(
        ...
        match val {
            ...
            // indexed elements come from user-set properties (e.g. fun[0] = 12).
            JsValue::Function(func) => {
    ...
    fn static_get_own_property_descriptor(obj: &JsValue, prop: &str) -> JsValue {
        ...
        match obj {
            ...
            JsValue::BoundHostFunction { .. } => match prop {
                "name" => make_data(JsValue::String(String::new()), false, false, true),
                "length" => make_data(JsValue::Number(1.0), false, false, true),
                _ => JsValue::Undefined,
            },
...
mod tests {
    ...
    fn promise_resolving_functions_expose_builtin_metadata() {
        let program = crate::parse_script(
            ...
            let lengthDesc = Object.getOwnPropertyDescriptor(resolveLater, "length");
            let nameDesc = Object.getOwnPropertyDescriptor(resolveLater, "name");
            let names = Object.getOwnPropertyNames(resolveLater);
            document.getElementById("result").textContent =
                String(Object.isExtensible(resolveLater)) + "/" +
                String(resolveLater.length) + "/" +
                resolveLater.name + "/" +
                String(lengthDesc.writable) + "/" +
                String(lengthDesc.enumerable) + "/" +
                String(lengthDesc.configurable) + "/" +
                String(nameDesc.writable) + "/" +
                String(nameDesc.enumerable) + "/" +
                String(nameDesc.configurable) + "/" +
                names.join(",");
            "#,
        )
        .expect("script should parse");
        ...
        assert_eq!(
            ...
            vec![BrowserEffect::SetTextContent {
                element_id: "result".to_owned(),
                value: "true/1//false/false/true/false/false/true/length,name".to_owned(),
            }]
        );
    }

    #[test]
    fn promise_resolve_assimilates_thenable_object() {
        let program = crate::parse_script(
            r#"
            let output = "";
            Promise.resolve({
    ...
    #[test]
    fn promise_resolve_thenable_runs_then_in_microtask_order() {
        let program = crate::parse_script(
            r#"
            let output = "";
            let thenable = {
    ...
    #[test]
    fn promise_then_chain_assimilates_returned_thenable() {
        let program = crate::parse_script(
            r#"
            let output = "";
            Promise.resolve("A")
    ...
    #[test]
    fn promise_resolve_rejects_throwing_then_getter() {
        let program = crate::parse_script(
            r#"
            let output = "";
            let thenable = {};
            ...
            Promise.resolve(thenable).then(undefined, function (reason) {
                output = reason;
            });
            setTimeout(function () {
                document.getElementById("result").textContent = output;
    ...
    #[test]
    fn promise_resolve_rejects_self_resolution() {
        let program = crate::parse_script(
            r#"
            let output = "";
            let resolveLater;
        ...
        assert_eq!(
            ...
            vec![BrowserEffect::SetTextContent {
                element_id: "result".to_owned(),
                value: "TypeError".to_owned(),
            }]
        );
    }

    #[test]
    fn promise_resolving_functions_ignore_later_calls_after_adopting_pending_promise() {
        let program = crate::parse_script(
            r#"
            let resolveInner;
            let output = "";
            ...
            setTimeout(function () { resolveInner("ok"); }, 0);
            setTimeout(function () {
                document.getElementById("result").textContent = output;
            }, 0);
            "#,
    ...
    #[test]
    fn promise_all_consumes_custom_iterator_input() {
        let program = crate::parse_script(
            r#"
            let output = "";
            let iterable = {};
    ...
    #[test]
    fn reflect_property_and_prototype_dispatch() {
        let program = crate::parse_script(
            r#"
            let proto = { inherited: "P" };
            let obj = {};
            let setProto = Reflect.setPrototypeOf(obj, proto);
            let protoValue = Reflect.getPrototypeOf(obj).inherited;
            let defined = Reflect.defineProperty(obj, "own", {
                value: "O",
                configurable: true
            });
            let setOwn = Reflect.set(obj, "newOwn", "N");
            document.getElementById("result").textContent =
                String(setProto) + "/" + protoValue + "/" +
                String(defined) + "/" + String(Reflect.has(obj, "inherited")) + "/" +
                Reflect.get(obj, "inherited") + "/" + Reflect.get(obj, "own") + "/" +
                String(setOwn) + "/" + Reflect.get(obj, "newOwn");
            "#,
        )
        .expect("script should parse");
        ...
        assert_eq!(
            ...
            vec![BrowserEffect::SetTextContent {
                element_id: "result".to_owned(),
                value: "true/P/true/true/P/O/true/N".to_owned(),
            }]
        );
    }

    #[test]
    fn reflect_define_property_reports_false_for_non_configurable_change() {
        let program = crate::parse_script(
            r#"
            let obj = {};
            ...
            let changed = Reflect.defineProperty(obj, "locked", { value: "B" });
            document.getElementById("result").textContent =
                String(changed) + "/" + Reflect.get(obj, "locked");
            "#,
        )
        .expect("script should parse");
        ...
        assert_eq!(
            ...
            vec![BrowserEffect::SetTextContent {
                element_id: "result".to_owned(),
                value: "false/A".to_owned(),
            }]
        );
    }

    #[test]
    fn reflect_apply_calls_function_with_this_and_arguments() {
        let program = crate::parse_script(
            r#"
            let receiver = { prefix: "R" };
            function join(a, b) {
                return this.prefix + a + b;
            }
            document.getElementById("result").textContent =
                Reflect.apply(join, receiver, ["A", "B"]);
            "#,
        )
        .expect("script should parse");
    ...
    #[test]
    fn async_pending_await_inside_if_condition_resumes_selected_branch() {
        let program = crate::parse_script(
            r#"
            let resolveLater;
            let output = "";
    ...
    #[test]
    fn async_pending_await_inside_do_while_resumes_loop_cursor() {
        let program = crate::parse_script(
            r#"
            let resolveLater;
            let output = "";
    ...
    #[test]
    fn async_pending_await_inside_for_resumes_loop_cursor() {
        let program = crate::parse_script(
            r#"
            let resolveLater;
            let output = "";
    ...
    #[test]
    fn async_pending_await_in_for_init_resumes_without_rerunning_loop() {
        let program = crate::parse_script(
            r#"
            let resolveLater;
            let output = "";
            ...
            run().then(function () { output = output + "E"; });
            output = output + "S";
            setTimeout(function () { resolveLater(1); }, 0);
            setTimeout(function () {
                document.getElementById("result").textContent = output;
            }, 0);
            "#,
        ...
        assert_eq!(
            ...
            vec![BrowserEffect::SetTextContent {
                element_id: "result".to_owned(),
                value: "SB1B2DE".to_owned(),
            }]
        );
    }

    #[test]
    fn async_pending_await_in_for_update_resumes_without_rerunning_body() {
        let program = crate::parse_script(
            r#"
            let resolveLater;
            let output = "";
        ...
        assert_eq!(
            ...
            vec![BrowserEffect::SetTextContent {
                element_id: "result".to_owned(),
                value: "B0SB1B2DE".to_owned(),
            }]
        );
    }

    #[test]
    fn async_pending_await_in_for_test_resumes_without_skipping_body() {
        let program = crate::parse_script(
            r#"
            let resolveLater;
            let output = "";
        ...
        assert_eq!(
            ...
            vec![BrowserEffect::SetTextContent {
                element_id: "result".to_owned(),
                value: "SB0DE".to_owned(),
            }]
        );
    }

    #[test]
    fn async_pending_await_in_for_in_object_resumes_iteration_setup() {
        let program = crate::parse_script(
            r#"
            let resolveLater;
            let output = "";
            ...
            run().then(function () { output = output + "E"; });
            output = output + "S";
            setTimeout(function () { resolveLater({ a: 1, b: 2 }); }, 0);
            setTimeout(function () {
                document.getElementById("result").textContent = output;
            }, 0);
            "#,
        ...
        assert_eq!(
            ...
            vec![BrowserEffect::SetTextContent {
                element_id: "result".to_owned(),
                value: "SabDE".to_owned(),
            }]
        );
    }

    #[test]
    fn async_pending_await_in_for_of_iterable_resumes_iteration_setup() {
        let program = crate::parse_script(
            r#"
            let resolveLater;
            let output = "";
            ...
            run().then(function () { output = output + "E"; });
            output = output + "S";
            setTimeout(function () { resolveLater(["A", "B"]); }, 0);
            setTimeout(function () {
                document.getElementById("result").textContent = output;
            }, 0);
            "#,
    ...
    #[test]
    fn async_pending_await_inside_for_in_resumes_loop_cursor() {
        let program = crate::parse_script(
            r#"
            let resolveLater;
            let output = "";
        ...
        assert_eq!(
            ...
            vec![BrowserEffect::SetTextContent {
                element_id: "result".to_owned(),
                value: "BaSABbBcCBdDE".to_owned(),
            }]
        );
    }

    #[test]
    fn async_pending_await_in_call_argument_resumes_call_expression() {
        let program = crate::parse_script(
            r#"
            let resolveLater;
            let output = "";
    ...
    #[test]
    fn generator_yield_in_call_argument_resumes_call_expression() {
        let program = crate::parse_script(
            r#"
            function join(a, b, c) {
                return a + b + c;
            ...
            function* gen() {
                let value = join("A", yield "pause", "C");
                yield value;
            }
            let g = gen();
            let r1 = g.next();
    ...
    #[test]
    fn generator_pending_yield_inside_if_condition_resumes_selected_branch() {
        let program = crate::parse_script(
            r#"
            let output = "";
            function* gen() {
        ...
        assert_eq!(
            ...
            vec![BrowserEffect::SetTextContent {
                element_id: "result".to_owned(),
                value: "A/ATD/flag/true".to_owned(),
            }]
        );
    }
    ...
    #[test]
    fn generator_yield_inside_do_while_resumes_loop_cursor() {
        let program = crate::parse_script(
            r#"
            let output = "";
            function* gen() {
    ...
    #[test]
    fn generator_yield_inside_for_resumes_loop_cursor() {
        let program = crate::parse_script(
            r#"
            let output = "";
            function* gen() {
    ...
    fn labeled_break_exits_outer_loop() {
        let effects = run(r#"
            ...
            "#);
        assert_eq!(effects, vec![text("result", "0:0,0:1,0:2,1:0,")]);
    }
    ...
    fn labeled_continue_continues_outer_loop() {
        let effects = run(r#"
            ...
            document.getElementById("result").textContent = out;
            "#);
        assert_eq!(effects, vec![text("result", "0:0,1:0,2:0,")]);
    }

    #[test]
    fn labeled_block_break_skips_to_after_block() {
        let effects = run(r#"
            let out = "";
            done: {
                out = out + "a";
            ...
            document.getElementById("result").textContent = out;
            "#);
        assert_eq!(effects, vec![text("result", "ac")]);
    }

    #[test]
    fn labeled_switch_break_exits_switch() {
        let effects = run(r#"
            let out = "";
            target: switch (1) {
                case 1:
            ...
            document.getElementById("result").textContent = out;
            "#);
        assert_eq!(effects, vec![text("result", "ad")]);
    }

    #[test]
    fn generator_labeled_break_after_yield_targets_outer_loop() {
        let program = crate::parse_script(
            r#"
            function* gen() {
                let out = "";
        ...
        assert_eq!(
            ...
            vec![BrowserEffect::SetTextContent {
                element_id: "result".to_owned(),
                value: "pause/0:0,/false".to_owned(),
            }]
        );
    }

    #[test]
    fn async_labeled_break_after_await_targets_outer_loop() {
        let program = crate::parse_script(
            r#"
            let resolveLater;
            let p = new Promise(function (resolve) { resolveLater = resolve; });
    ...
    #[test]
    fn class_prototype_method_inheritance() {
        let effects = run(r#"
            class Animal {
              constructor(name) { this.name = name; }
              speak() { return this.name + " makes a sound"; }
